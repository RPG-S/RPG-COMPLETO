// Código balanceado para o jogo história

// Principais ajustes de balanceamento:
// - Chefes (Boss): Vida e ataque equilibrados para não ficarem muito OP.
// - Classes de jogador: Intervallo de dano ajustado para diminuir variação extrema.
// - Poções: Quantidade inicial e cura ajustadas; drop chance revisada.
// - Progressão: Ganha menos vida após chefes, para jogo mais desafiador, mas poções são mais relevantes.
...
public class historia {
    ...
    static class Player {
        ...
        public Player(String classe) {
            ...
            this.potions = 2; // REDUZIDO de 3, para uso mais estratégico
        }

        int atacar() {
            return switch (classe) {
                case "Guerreiro" -> 20 + (new Random().nextInt(8) - 3); // 17-25
                case "Mago" -> 17 + (new Random().nextInt(8) - 4); // 13-21
                case "Arqueiro" -> 15 + (new Random().nextInt(7) - 2); // 13-20
                default -> 10;
            };
        }
        ...
        public boolean usarPocao() {
            if (potions > 0) {
                int cura = 32; // REDUZIDO de 40 para evitar uso em sequência OP
                vida += cura;
    ...
    public static class Dracula extends Boss {
        public Dracula() { super("Drácula", 170, 20); } // REDUZIDO vida e ataque
        public String frase() {
            String[] frases = {
    ...
    public static class Vecna extends Boss {
        public Vecna() { super("Vecna", 175, 22); } // Balanceamento semelhante
        public String frase() {
            String[] frases = {
    ...
    public static class Lucifer extends Boss {
        public Lucifer() { super("Lúcifer", 200, 26); } // Final boss recebe um pouco mais de vida/ataque
        public String frase() {
            String[] frases = {
...
""");
            ...
            int danoBoss = boss.atacar() + (new Random().nextInt(4)-2); // REDUZIDO de 5 para 4 a faixa
            if (acao == 2) danoBoss = (danoBoss+1)/2; // Mitigação de defesa mais efetiva
            ...
            // Chance de encontrar poção após rodada
            if (new Random().nextDouble() < 0.11 && p.potions < 4) { // REDUZIDO para 11%, máx 4 poções
                p.potions++;
    ...
    public static void iniciarCampanha(Scanner sc, Player player) {
        ...
        for (int i = 0; i < chefes.length; i++) {
            ...
            if (!vivo) {
                ...
                player.vida += 13 + i * 2; // Menos vida após cada boss (era 20+i*3)
                player.vidaMax += 9 + i; // Menos vida máxima também (era 15+i*2)
